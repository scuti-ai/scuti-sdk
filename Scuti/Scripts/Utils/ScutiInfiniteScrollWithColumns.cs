...
namespace Scuti.UI
{
    ...
    public class ScutiInfiniteScrollWithColumns : MonoBehaviour
    {
		...
		[SerializeField] private ColumnSystem columnSystem;
        ...
        {
			var children = columnSystem.GetChildren();

			for (int i = 0; i < children.Count; i++)
            {
                items.Add(children[i].GetComponent<RectTransform>());
            }

            _itemCount = children.Count;
        }
        ...
        public void CleanupItems()
        {
            ...
            {
                if(!items.Contains(children[i].GetComponent<RectTransform>()))
                {
                    Transform child = children[i].GetComponent<RectTransform>();
		...
		private void SetParentToObject(Transform obj)
		{
			//columnSystem.SetObjectParent(obj);
		}
