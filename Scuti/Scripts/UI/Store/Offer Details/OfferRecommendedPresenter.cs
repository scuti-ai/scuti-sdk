...
namespace Scuti.UI {
    public class OfferRecommendedPresenter : Presenter<OfferRecommendedPresenter.Model> {
        ...
        public async void SearchForRecommendations(string shopName, string currentOfferId)
        {
            ...
            {
                var offers = await ScutiNetClient.Instance.Offer.GetOffers(new List<CampaignType> { CampaignType.Product, CampaignType.ProductListing }, OfferService.MediaType.Product, FILTER_TYPE.In, null, shopName, null, 0, MaxRecommendations+1);
